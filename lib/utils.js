...
const useMethods = (routeTable, target) => {
  const registerMethod = (path, handler, ...names) => {
  ...
  for (let i = 0; i < methods.length; i += 1) {
    ...
    if (method !== 'all' && !routeTable[method]) {
      Object.assign(routeTable, { [method]: [] });
    }

    const controller = async (path, handler) => {
      if (method === 'all') {
        registerMethod(path, handler, ...methods.filter((m) => m !== 'all'));
      } else {
        registerMethod(path, handler, method);
      }
    };

    Object.assign(target, { [method]: controller });
  }
};

const useRoutes = async (routeTable, req, res, callback, prefix = '') => {
  req.params = {};
...
const useMiddlewares = (stacks, ...args) => {
