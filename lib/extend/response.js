...
res.send = function (data) {
  ...
  if (availableCache) {
    ...
    if (
      cachedData &&
      this.request.method.includes(cachedData.value.method) &&
      cachedData.value.raw === bodyAsString
    ) {
      this.statusCode = statusCode;

      for (const key in cachedData.value.headers) {
        if (Object.hasOwnProperty.call(cachedData.value.headers, key)) {
          const value = cachedData.value.headers[key];
          headers[key] = value;
        }
      }
