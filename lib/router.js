const { regexToParams, regexToString } = require('./utils');
...
class Router {
  ...
  addRoute(method, route, ...handlers) {
    ...
    if (this.prefix) {
      ...
      if (prefix instanceof RegExp) {
        prefix = regexToString(prefix, true);
      }

      if (path instanceof RegExp) {
        path = regexToString(path);
      }
  ...
  addMiddleware() {
    ...
    this.routes.push({
      ...
      method: '',
      isMiddleware: true,
      middlewarePath: this.prefix ? route : undefined,
    });
